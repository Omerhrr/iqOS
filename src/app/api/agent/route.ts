...
const TOOLS: ToolSpec[] = [
  ...
  },
  {
    ...
    run: (a) => {
      ...
      if (a.strategy) p.set('strategy', String(a.strategy))
      if (a.limit) p.set('limit', String(a.limit))
      return coreGet(`/calibration?${p.toString()}`)
    },
  },
  {
    name: 'asset_sweep',
  ...
  },
  {
    ...
    description: 'Run EVERY registered strategy (14 incl. vsk-synthesis/tsk-synthesis/ensemble-vote) on one asset+tf with default params via the real binary settlement engine, then rank them by netPnl / winRate / profitFactor / sharpe. The "which edge actually fits THIS instrument" answer in one call. Heavy (14 backtests) - follow up with optimize_strategy + walkforward on the winner.',
